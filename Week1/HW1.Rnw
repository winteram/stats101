...
@
\firstpageheader{Name:\enspace\makebox[2in]{\hrulefill} \\ FPHDR
  \\ COURSENO}{\Large Homework 1 } {\ifprintanswers \textbf{Answer Key} \fi
  \\ UNIQUEKEY}
\runningheader{RNHDR \\ BT221}{\Large Homework 1 }{\ifprintanswers
...
@ 
  ...
  http://bit.ly/qt1L6b to see if anyone has already selected that data
  set.  If not, go to http://bit.ly/oObGms to post your data set so no one
  else will use it.
    \begin{parts}
      \part[2] What is an elementary unit for this data set? 
      \part[2] How many variables are there in this data set?
